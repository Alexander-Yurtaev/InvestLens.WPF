...
namespace InvestLens.Tests
{
    public class NavigationViewModelTests
    {
        ...
        public void MenuItemsShouldBeInitializedCorrect()
        {
            ...
            Assert.NotNull(dashboard.MenuItems);
            Assert.Empty(dashboard.MenuItems);
            ...
            Assert.NotNull(portfolios.MenuItems);
            Assert.Equal(3, portfolios.MenuItems.Count);
            ...
            Assert.NotNull(dictionaries.MenuItems);
            Assert.Equal(2, dictionaries.MenuItems.Count);

            var moex = dictionaries.MenuItems[0];
            Assert.NotNull(moex);
            Assert.Equal("MOEX", moex.Header);
            Assert.NotNull(moex.MenuItems);
            Assert.Equal(2, moex.MenuItems.Count);

            var dohod = dictionaries.MenuItems[1];
            Assert.NotNull(dohod);
            Assert.Equal("Dohod.ru", dohod.Header);
            Assert.NotNull(dohod.MenuItems);
            Assert.Single(dohod.MenuItems);
            ...
            Assert.NotNull(downloader.MenuItems);
            Assert.Empty(downloader.MenuItems);
            ...
            Assert.NotNull(scheduler.MenuItems);
            Assert.Empty(scheduler.MenuItems);
            ...
            Assert.NotNull(settings.MenuItems);
            Assert.Equal(2, settings.MenuItems.Count);
        }
    }
}
