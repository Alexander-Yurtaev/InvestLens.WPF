...
База данных SQLite создается автоматически при первом запуске приложения через `Database.EnsureCreated()`. Все таблицы создаются на основе моделей Entity Framework.

> **Важно**: При изменении модели данных удалите файл базы данных вручную — `EnsureCreated()` не обновляет схему.
